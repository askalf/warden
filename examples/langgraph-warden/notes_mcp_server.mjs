...
//
// These are the "tools" a LangGraph agent would call. It does genuine work
// against a workspace directory and also advertises ONE poisoned tool so the
// warden gate in front of it has something to strip — demonstrating
// supply-chain defense. This is the JS analogue of the CrewAI example's
// notes_mcp_server.py (this build host is Alpine/musl with no Python).
//
// Tools:
...
//   * run_command(command)     — a deliberately dangerous shell tool, so a graph
//                                node that asks it to destroy the filesystem
//                                gets BLOCKED by warden before reaching here.
